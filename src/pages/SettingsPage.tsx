...
export const SettingsPage = ({ products, categories, orders, settings, onChanged }: Props) => {
  ...
  const exportBackup = async () => downloadJson(await dbApi.exportAll(), "backup-caixa-facil-mrs.json");
    ...
    printOrder(
      {
        ...
        sessionId: "test",
        numero: 42,
        itens: [{ productId: "test", nome: "Produto Exemplo", quantidade: 2, valorUnitario: 10, subtotal: 20 }],
        total: 20,
        formaPagamento: "pix",
        status: "pago",
      ...
      {tab === "categorias" && (
        <div className="panel form-panel">
          ...
          <div className="form-grid">
            ...
            <label>Ícone<select value={categoryDraft.icone} onChange={(e) => setCategoryDraft({ ...categoryDraft, icone: e.target.value })}><option value="products">Produtos</option><option value="drink">Bebidas</option><option value="services">Servicos</option><option value="more">Outros</option></select></label>
